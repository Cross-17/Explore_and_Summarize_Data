...
ggplot(aes(x = quality), data = wine)  + geom_histogram(aes(y = (..count..)/sum(..count..))) + ggtitle("Plot of quality distribution") + ylab('percentage distribution')
...
```{r echo=FALSE}
  ...
  ggplot(aes(x = citric.acid), data = wine) + geom_histogram(binwidth = 0.01) + ggtitle('Citric acid distribution')
...
p2 <- ggplot(aes(x = total.sulfur.dioxide), data = wine) + geom_histogram(binwidth = 0.05) + ggtitle('Total sulfur dioxide distribution log10') + scale_x_log10()
...
ggplot(aes(x = alcohol, y = volatile.acidity), data = wine) +
  geom_jitter(aes(color = factor(quality) )) + scale_color_brewer(type = 'qual')+
